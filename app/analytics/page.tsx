...
export default function SummaryDashboard() {
  ...
  const chartInstances = useRef<{ pie1: any; pie2: any; bar: any }>({
    pie1: null,
    pie2: null,
    bar: null,
  });
  ...
  useEffect(() => {
    ...
    const page = table.closest(".Analytics-paper");
    if (!page) return;
    const tableHeight = table.scrollHeight;
    const pageHeight = (page as HTMLElement).clientHeight;
    if (tableHeight > pageHeight) {
      const scale = pageHeight / tableHeight;
      setTableScale(scale);
    } else {
      setTableScale(1);
    }
  }, [filteredRecords, paperSize]);

  useEffect(() => {
    if (!tableRef.current) return;
    const table = tableRef.current;
    const page = table.closest(".Analytics-paper") as HTMLElement;
    if (!page) return;
    const pagePadding = 96;
    const pageHeightPadding = 96;
    const availableWidth = page.clientWidth - pagePadding;
    const availableHeight = page.clientHeight - pageHeightPadding;
    ...
    async function loadData() {
      try {
        ...
        const records = querySnapshot.docs.map((doc) => {
          ...
          let d = String(
            data.disability || data.r9_disability || "None",
          ).trim();
          if (!d || d.toLowerCase() === "none" || d === "0" || d === "") {
            d = "None";
          } else if (d.toLowerCase().includes("physical")) {
            d = "Physical";
          } else if (d.toLowerCase().includes("intellectual")) {
            d = "Intellectual";
          }

          let i = String(data.illness || data.r10_illness || "None").trim();
          if (!i || i.toLowerCase() === "none" || i === "0") {
            i = "None";
          } else {
            // Normalize to title case to prevent "paralysis" vs "Paralysis" duplicates
  ...
  const { chartData, paragraphs, fullTableData, categoryTables, places } =
    useMemo(() => {
      const dCount: any = {};
      const iCount: any = {};
      const pCount: any = {};
      ...
      const places = [...new Set(filteredRecords.map((r) => r.place))].sort(
      ...
      if (totalPop === 0) {
        paras.push(
          "<p>No records match the current filter criteria. Please adjust your parameters to generate a summary.</p>",
        );
      } else {
        paras.push(`<h3 style="color: #512da8; margin-bottom: 8px; font-size: 16px;">I. Population Overview</h3>
        <p>The dataset includes <b>${totalPop}</b> individuals from <b>${places.length}</b> based on the selected data. This report provides an overview of the population, including their gender, location, disabilities, and health conditions.</p>`);

        const totalMales = filteredRecords.filter(
          (r) => r.sex === "Male",
        ).length;
        const totalFemales = filteredRecords.filter(
          (r) => r.sex === "Female",
        ).length;
        paras.push(`<h3 style="color: #512da8; margin-bottom: 8px; font-size: 16px;">II. Gender Distribution</h3>
        <p>The population consists of <b>${totalMales}</b> males <b>(${pct(totalMales, totalPop)}%)</b>, and <b>${totalFemales}</b> females <b>(${pct(totalFemales, totalPop)}%)</b>.</p>`);

        const placesCount = Object.entries(pCount).sort(
          (a: any, b: any) => b[1] - a[1],
        );
        if (placesCount.length > 0) {
          const topPlaces = placesCount
            .slice(0, 5)
            .map((p) => `<b>${p[0]}</b> (${p[1]} individuals)`)
            .join(", ");
          const bottomPlaces = placesCount
            .slice(-3)
            .map((p) => `<b>${p[0]}</b> (${p[1]} individuals)`)
            .join(", ");
          paras.push(`<h3 style="color: #512da8; margin-bottom: 8px; font-size: 16px;">III. Population by Barangay</h3>
          <p>The number of individuals varies across barangays. The highest populations are in ${topPlaces}. The lowest counts are in ${bottomPlaces}.</p>`);
        }

        const withDisabilities = filteredRecords.filter(
          (r) => r.disabilities !== "None",
        );
        if (withDisabilities.length > 0) {
          const dCountMap: any = {};
          withDisabilities.forEach(
            (r) =>
              (dCountMap[r.disabilities] =
                (dCountMap[r.disabilities] || 0) + 1),
          );
          const topDisabilities = Object.entries(dCountMap)
            .sort((a: any, b: any) => b[1] - a[1])
            .slice(0, 5)
            .map((d: any) => `<b>${d[0]}</b> (${d[1]} cases)`)
            .join(", ");
          paras.push(`<h3 style="color: #512da8; margin-bottom: 8px; font-size: 16px;">IV. Disability Summary</h3>
          <p>A total of <b>${withDisabilities.length}</b> individuals <b>(${pct(withDisabilities.length, totalPop)}%)</b> reported having a disability or special need. The most common conditions are ${topDisabilities}.</p>`);
        } else {
          paras.push(
            `<h3 style="color: #512da8; font-size: 16px;">IV. Disability Summary</h3><p>No disabilities or special needs were reported within the current parameters.</p>`,
          );
        }
        ...
        if (withIllness.length > 0) {
          const iCountMap: any = {};
          withIllness.forEach(
            (r) => (iCountMap[r.illness] = (iCountMap[r.illness] || 0) + 1),
          );
          const topIllnesses = Object.entries(iCountMap)
            .sort((a: any, b: any) => b[1] - a[1])
            .slice(0, 5)
            .map((i: any) => `<b>${i[0]}</b> (${i[1]} cases)`)
            .join(", ");
          paras.push(`<h3 style="color: #512da8; margin-bottom: 8px; font-size: 16px;">V. Critical Illness Summary</h3>
          <p>A total of <b>${withIllness.length}</b> individuals (<b>${pct(withIllness.length, totalPop)}%</b>) reported serious health conditions. The most common illnesses are ${topIllnesses}.</p>`);
        } else {
          paras.push(
            `<h3 style="color: #512da8; margin-bottom: 8px; font-size: 16px;">V. Critical Illness Summary</h3><p>No critical illnesses were reported within the current parameters.</p>`,
          );
        }

        const highRisk = filteredRecords.filter(
          (r) => r.disabilities !== "None" && r.illness !== "None",
        );
        if (highRisk.length > 0) {
          paras.push(`<h3 style="color: #e53935; margin-bottom: 8px; font-size: 16px;">VI. Disability and Illness Cases</h3>
          <p>A total of <b>${highRisk.length}</b> individuals (<b>${pct(highRisk.length, totalPop)}%</b>) have both a disability and a serious illness.</p>`);
        }
      }

      const sexes = [...new Set(filteredRecords.map((r) => r.sex))].sort();
      const dTypes = [...new Set(filteredRecords.map((r) => r.disabilities))]
        .filter((d) => d && d.trim() !== "")
        .sort();
      const iTypes = [
        ...new Set(
          filteredRecords
            .map((r) => r.illness)
            .filter((i) => i && i !== "0" && i.trim() !== ""),
        ),
      ].sort();

      const catTablesData = ["place", "sex", "disabilities", "illness"].map(
        (cat) => {
          const counts: any = {};
          filteredRecords.forEach(
            (r) =>
              (counts[r[cat as keyof typeof r] || "None"] =
                (counts[r[cat as keyof typeof r] || "None"] || 0) + 1),
          );
          return {
            category: cat,
            label: {
            ...
            counts: Object.entries(counts) as [string, number][],
          };
        },
      );
  ...
  useEffect(() => {
    ...
    if (pie1Ref.current) {
      chartInstances.current.pie1 = new Chart(pie1Ref.current, {
        type: "pie",
        data: {
          labels: Object.keys(chartData.disabilities),
          datasets: [
            {
              data: Object.values(chartData.disabilities),
    ...
    if (pie2Ref.current) {
      chartInstances.current.pie2 = new Chart(pie2Ref.current, {
        type: "pie",
        data: {
          labels: Object.keys(chartData.illness),
          datasets: [
            {
              data: Object.values(chartData.illness),
              backgroundColor: pieColors,
            },
          ],
        },
    ...
    if (barRef.current) {
      chartInstances.current.bar = new Chart(barRef.current, {
        type: "bar",
        data: {
          labels: Object.keys(chartData.place),
          datasets: [
            {
              label: "Population",
              data: Object.values(chartData.place),
    ...
    const filtered = globalRecords.filter((record) => {
      return (
        ...
        (!filters.disabilities.length ||
          filters.disabilities.includes(record.disabilities)) &&
  ...
  return (
    <>
      ...
      <Script
        ...
        onReady={() => setH2cLoaded(true)}
      />
      <Script
        src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"
        onReady={() => setJsPdfLoaded(true)}
      />

      {/* Dynamic @page rule for correct paper size when printing */}
      <style
      ...
      <style
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: "Segoe UI", Arial, sans-serif; }
        ...
        .analytics-root { 
          ...
          background-color: #0f111a; display: block; overflow: auto; 
        }
        
        .page-container { 
          background-color: white; min-width: 100%; width: fit-content; min-height: 100%; 
          box-shadow: 0 10px 30px rgba(0,0,0,0.5); display: block; text-align: center; 
          padding-top: 40px; padding-bottom: 40px; 
        }

        #pagesContainer { 
          display: inline-flex; flex-direction: column; align-items: flex-start; 
          gap: 40px; text-align: left; padding: 0 40px; 
        }
        ...
        .size { border: none; font-weight: bold; cursor: pointer; color: black; transition: 0.2s; width: 94px; height: 25px; border-radius: 20px; }
        ...
        @media print {
          html, body, #__next, #__next > div { 
            ...
            padding: 0 !important; overflow: visible !important; background: white !important; width: 100% !important;
          }
          ...
          #pagesContainer { 
            transform: scale(1) !important; gap: 0 !important; display: block !important; 
      ...
      <div className="analytics-root">
        <div
          ...
          style={{
            paddingRight: isFilterOpen ? "360px" : "0",
        ...
        >
          ...
          <div className="action-bar">
            ...
            <div
              className="dropdown"
              id="paperSize"
              onMouseLeave={() => setPaperDropdownOpen(false)}
            >
              <button
                className={`btn btn-paper ${paperDropdownOpen ? "show" : ""}`}
                onClick={() => setPaperDropdownOpen(!paperDropdownOpen)}
              >
                Paper Size
              </button>
              <div
                className={`paper-choice-dropdown ${paperDropdownOpen ? "show" : ""}`}
              >
                <button
                  className="size btn-a4"
                  onClick={() => {
                    setPaperSize("a4");
            ...
            <div style={{ display: "flex", gap: "15px", alignItems: "center" }}>
              ...
              >
                <img
                  ...
                  alt="layout icon"
                  style={{
                    filter: layout2Pages ? "brightness(0) invert(1)" : "none",
                  }}
                  onError={(e) => {
                    e.currentTarget.src = "/window.svg";
                    e.currentTarget.style.filter = layout2Pages
                      ? "invert(1)"
                      : "none";
                  }}
                />
          ...
          <div
            id="pagesContainer"
            className={layout2Pages ? "two-page" : ""}
            style={
              {
                zoom: zoom,
                "--page-width": currentPaper.width,
                "--page-height": currentPaper.height,
              ...
              <>
                ...
                >
                  ...
                  >
                    ...
                    <div className="chartCard" style={{ flex: 1 }}>
                ...
                {paragraphs.map((paraChunk, idx) => (
                  ...
                  >
                    <h1>Summary Report {idx > 0 ? `(Part ${idx + 1})` : ""}</h1>
                    <div
                      style={{
                        display: "flex",
                        flexDirection: "column",
                  ...
                  <table>
                    ...
                    <thead>
                      <tr>
                        ...
                        <th colSpan={fullTableData.dTypes.length}>
                          Disabilities
                        </th>
                      </tr>
                      <tr>
                        {fullTableData.dTypes.map((d) => (
                          <th key={d}>{d}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {places.map((place) => {
                        const placeRecords = filteredRecords.filter(
                          (r) => r.place === place,
                        );
                        const activeSexes = fullTableData.sexes.filter((sex) =>
                          placeRecords.some((r) => r.sex === sex),
                        );
                        return activeSexes.map((sex, index) => {
                          const sexRecords = placeRecords.filter(
                            (r) => r.sex === sex,
                          );
                          return (
                            <tr key={`${place}-${sex}-disability`}>
                              {index === 0 && (
                                <td rowSpan={activeSexes.length}>
                                  <b>{place}</b>
                                </td>
                              )}
                              <td>{sex}</td>
                              <td>
                                <b>{sexRecords.length}</b>
                              </td>
                              {fullTableData.dTypes.map((d) => (
                                <td key={d}>
                                  {
                ...
                {/* PAGE 4: ILLNESS TABLE */}
                <div className="Analytics-paper">
                  <h1>Critical Illness Summary</h1>
                  <table>
                    <colgroup>
                      <col style={{ width: "14%" }} />
                      <col style={{ width: "8%" }} />
                    ...
                    <thead>
                      <tr>
                        ...
                        <th rowSpan={2}>Sex</th>
                        <th rowSpan={2}>Population</th>
                        <th colSpan={fullTableData.iTypes.length}>
                          Critical Illness
                        </th>
                      </tr>
                      <tr>
                        {fullTableData.iTypes.map((i) => (
                          <th key={i}>{i}</th>
                        ))}
                      </tr>
                    </thead>
                          ...
                          return (
                            <tr key={`${place}-${sex}-illness`}>
                              ...
                              {fullTableData.iTypes.map((i) => (
                                <td key={i}>
                                  {
                                    sexRecords.filter((r) => r.illness === i)
                                      .length
                                  }
                                </td>
                              ))}
                ...
                {/* PAGE 5+: CATEGORY SUMMARY TABLES - grouped to fill pages */}
                {(() => {
                  const pages: (typeof categoryTables)[] = [];
                  let currentPage: typeof categoryTables = [];
                  let currentHeightUsed = 0;

                  // Letter page usable height estimate: 279mm - 2×0.5in padding - h1 title
                  const PAGE_USABLE_HEIGHT = 800;

                  const estimateTableHeight = (
                    catData: (typeof categoryTables)[0],
                  ) => {
                    const h3Height = 30;
                    ...
                    return (
                      h3Height +
                      tableHeaderHeight +
                      catData.counts.length * rowHeight +
                      gapHeight
                    );
                  };

                  categoryTables.forEach((catData) => {
                    const tableHeight = estimateTableHeight(catData);
                    if (
                      currentHeightUsed + tableHeight > PAGE_USABLE_HEIGHT &&
                        ...
                        {pageGroup.map((catData) => (
                          <div key={catData.category}>
                            ...
                            <table>
                              ...
                              <tbody>
                                {(catData.counts as [string, number][]).map(
                                  ([type, count]) => (
                                    <tr key={type}>
          ...
          <div className="filter-sidebar-wrapper">
            <div
              className={`filter-group-container ${isFilterOpen ? "" : "closed"}`}
            >
              <div className="filter-header-icons">
                <h3 style={{ margin: 0, color: "#2a1b3c", fontSize: "18px" }}>
                  Filter Options
                </h3>
                <button
              ...
              <div className="filter-scrollable-content">
                {/* Gender */}
                <div
                  className={`filter-category ${openCategories["sex"] ? "active" : ""}`}
                >
                  <div
                    className="category-header"
                    onClick={() => toggleAccordion("sex")}
                  >
                    <span>Gender</span>
                  ...
                  <div className="category-checklist">
                    {["Female", "Male"].map((opt) => (
                      <div
                        ...
                        key={opt}
                        onClick={() => handleFilterToggle("sex", opt)}
                      >
                        <input
                          type="checkbox"
                          value={opt}
                          checked={filters.sex.includes(opt)}
                          readOnly
                        />
                        <label>{opt}</label>
                      </div>
                ...
                {/* Barangay */}
                <div
                  className={`filter-category ${openCategories["place"] ? "active" : ""}`}
                >
                  <div
                    className="category-header"
                    onClick={() => toggleAccordion("place")}
                  >
                    <span>Barangay</span>
                    <span className="category-indicator">▼</span>
                  </div>
                  <div className="category-checklist">
                    {[
                      ...
                      <div
                        className="checkbox-item"
                        key={opt}
                        onClick={() => handleFilterToggle("place", opt)}
                      >
                        <input
                          type="checkbox"
                          value={opt}
                          checked={filters.place.includes(opt)}
                          readOnly
                        />
                        <label>{opt}</label>
                      </div>
                ...
                {/* Disability */}
                <div
                  className={`filter-category ${openCategories["disabilities"] ? "active" : ""}`}
                >
                  <div
                    className="category-header"
                    onClick={() => toggleAccordion("disabilities")}
                  >
                    <span>Disability/Special Needs</span>
                    <span className="category-indicator">▼</span>
                  </div>
                  <div className="category-checklist">
                    {[
                      ...
                      <div
                        className="checkbox-item"
                        key={opt}
                        onClick={() => handleFilterToggle("disabilities", opt)}
                      >
                        <input
                          type="checkbox"
                          value={opt}
                          checked={filters.disabilities.includes(opt)}
                          readOnly
                        />
                        <label>{opt}</label>
                      </div>
                ...
                {/* Illness */}
                <div
                  className={`filter-category ${openCategories["illness"] ? "active" : ""}`}
                >
                  <div
                    className="category-header"
                    onClick={() => toggleAccordion("illness")}
                  >
                  ...
                  <div className="category-checklist">
                    {[
                      ...
                      <div
                        className="checkbox-item"
                        key={opt}
                        onClick={() => handleFilterToggle("illness", opt)}
                      >
                        <input
                          type="checkbox"
                          value={opt}
                          checked={filters.illness.includes(opt)}
                          readOnly
                        />
                        <label>{opt}</label>
                      </div>
