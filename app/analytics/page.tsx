...
export default function SummaryDashboard() {
  ...
  const { chartData, paragraphs, fullTableData, categoryTables, chunkedPlacesList } = useMemo(() => {
    ...
    const validCatTables = catTablesData.filter(cat => cat.counts.length > 0);
    const chunkedCatTables: any[][] = [];
    for (let i = 0; i < validCatTables.length; i += 2) {
      chunkedCatTables.push(validCatTables.slice(i, i + 2));
    }
  ...
  const downloadPDF = async () => {
    ...
    wrappers.forEach((el: any) => {
      el.dataset.maxHeight = el.style.maxHeight;
      el.dataset.overflowY = el.style.overflowY;
    ...
    for(let i=0; i<pages.length; i++){
      const isLandscape = pages[i].classList.contains("landscape-paper");
            ...
            {!isLoading && (
              <>
                ...
                {chunkedPlacesList.map((placeChunk, pageIdx) => {
                  ...
                  const validPlaces = placeChunk.filter(place => 
                    fullTableData.sexes.some(sex => 
                      filteredRecords.some(r => r.place === place && r.sex === sex)
                    )
                  );
                  if (validPlaces.length === 0) return null;

                  return (
                    <div key={`full-table-page-${pageIdx}`} className="Analytics-paper landscape-paper full-table-paper" style={{ width: currentPaper.height, height: currentPaper.width }}>
                      ...
                      <table>
                        ...
                        <tbody>
                          {validPlaces.map(place => {
                             const validSexes = fullTableData.sexes.filter(sex => 
                                filteredRecords.some(r => r.place === place && r.sex === sex)
                             );
                             
                             if (validSexes.length === 0) return null;

                             return validSexes.map((sex, index) => {
                                const sexRecords = filteredRecords.filter(r => r.place === place && r.sex === sex);
                                return (
                                  <tr key={`${place}-${sex}`}>
                                    {index === 0 && <td rowSpan={validSexes.length} style={{verticalAlign: 'middle'}}><b>{place}</b></td>}
                                    <td>{sex}</td>
                ...
                {/* ✅ CATEGORY TABLES PAGES - Landscape & Stacked vertically */}
                {categoryTables.map((catChunk, pageIdx) => (
                  <div key={`cat-table-page-${pageIdx}`} className="Analytics-paper landscape-paper" style={{ 
                    ...
                    <h1 style={{ width: '100%', textAlign: 'center', borderBottom: 'none', marginBottom: '30px' }}>
                      Category Tables {pageIdx > 0 ? `(Part ${pageIdx + 1})` : ''}
                    </h1>
                      ...
                      {catChunk.map((catData: any, idx: number) => (
                        <div key={idx} className="category-table-wrapper" style={{ width: '100%', maxHeight: '350px', overflowY: 'auto', paddingRight: '10px' }}>
                          <table style={{ width: '100%', fontSize: '15px', border: '2px solid #000' }}>
                            <thead>
                              <tr>
                                <th style={{ backgroundColor: '#f2f2f2', padding: '12px' }}>{catData.label}</th>
                                <th style={{ backgroundColor: '#f2f2f2', padding: '12px' }}>Population</th>
                              </tr>
                            </thead>
                            <tbody>
                              {catData.counts.map(([type, count]: any) => (
                                <tr key={type}>
                                  <td style={{ textAlign: 'center', padding: '10px' }}>{type}</td>
                                  <td style={{ textAlign: 'center', padding: '10px' }}><b>{count}</b></td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
