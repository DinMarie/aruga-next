  ...
  return (
    <div className="tablePageRoot">
      <style dangerouslySetInnerHTML={{__html: `
        .tablePageRoot { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Arial, sans-serif; background-color: #ffffff; display: flex; justify-content: center; min-height: 100vh; }
        .tablePageRoot * { box-sizing: border-box; }
        /* Reverted to standard paper layout width */
        .tablePageRoot .page-container { background-color: white; width: 100%; max-width: 100%; box-shadow: none; display: flex; flex-direction: column; min-height: 100vh; }
        ...
        .tablePageRoot .main-layout { display: flex; flex: 1; min-height: 0; position: relative; height: calc(100vh - 80px); }
        .tablePageRoot .side-menu { width: 320px; background-color: #f4f0fa; border-right: 2px solid #cbbade; display: flex; flex-direction: column; transition: width 0.3s ease; overflow: hidden; height: 100%; }
        ...
        .tablePageRoot .main-content { flex: 1; display: flex; flex-direction: column; min-width: 0; background: white; transition: margin-left 0.3s ease; }
        .tablePageRoot .action-bar { padding: 20px 25px; display: flex; gap: 12px; align-items: center; flex-wrap: wrap; background: white; }
        ...
        .tablePageRoot .table-area { padding: 0 25px 20px 25px; flex: 1; overflow-x: auto; }
        .tablePageRoot .table-title { font-weight: bold; margin-bottom: 15px; font-size: 1.2rem; color: #333; }
        ...
        <div className="main-layout">
                    ...
                    {Object.keys(FILTER_OPTIONS).map(catKey => (
                      <div className="filter-category" key={catKey}>
                          <div className="category-header" onClick={() => toggleCategory(catKey)}>
                              <span style={{textTransform: 'capitalize'}}>{catKey}</span><span className="category-indicator">{openCategories[catKey] ? '▲' : '▼'}</span>
                          </div>
            ...
            <div className="main-content">
                <div className="action-bar">
                    <button className="btn-filter" onClick={() => setIsSideMenuCollapsed(!isSideMenuCollapsed)}>{isSideMenuCollapsed ? '☰ FILTER' : '◀ FILTER'}</button>
                ...
                <div className="table-area">
                    <div className="table-title">Summary of Informations (Showing {displayedProfiles.length === 0 ? 0 : startIndex + 1}-{Math.min(startIndex + ITEMS_PER_PAGE, displayedProfiles.length)} of {displayedProfiles.length})</div>
                    <table>
                        <thead>
                            <tr><th>Name</th><th>Birthday</th><th>Sex</th><th>Contact</th><th>Address</th><th>Religion</th><th>IP</th><th>Disability</th><th>Illness</th><th>Action</th></tr>
                        </thead>
                        <tbody>
                            {paginatedProfiles.length === 0 ? (
                                ...
                                <tr key={profile.id}>
                                    <td>{profile.name || 'N/A'}</td><td>{profile.birthday || 'N/A'}</td><td>{profile.sex || 'N/A'}</td><td>{profile.contact || 'N/A'}</td><td>{profile.address || 'N/A'}</td><td>{profile.religion || 'N/A'}</td><td>{profile.ip || 'N/A'}</td><td>{profile.disability || 'N/A'}</td><td>{profile.illness || 'N/A'}</td>
                ...
                <footer className="footer-controls">
                    ...
                    <div className="pagination">
                        <button className="pg-btn" onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1}>&lt;</button>
                        {Array.from({ length: totalPages }, (_, i) => i + 1).map(pageNum => (
                            <button key={pageNum} className={`pg-btn ${currentPage === pageNum ? 'active' : ''}`} onClick={() => setCurrentPage(pageNum)}>{pageNum}</button>
                        ))}
                        <button className="pg-btn" onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages}>&gt;</button>
                    </div>
                </footer>
            </div>
        </div>
