...
function FullDetailsEditContent() {
  ...
  if (isLoadingData) return <div style={{ padding: '50px', textAlign: 'center', fontSize: '20px' }}>Loading Edit Form...</div>;

  return (
    <div className="addWrapper">
      <Header subtitle="Edit Profiling and Assessment Details" />

      <div className="back-button" style={{ position: 'absolute', top: '90px', left: '20px' }}>
        <button onClick={() => router.push('/table')} style={{ width: '40px', height: '40px', borderRadius: '50%', border: '2px solid #8c6d8c', background: 'white', color: '#8c6d8c', cursor: 'pointer', fontWeight: 'bold' }}>←</button>
      </div>
        ...
        <form onSubmit={handleSubmit}>
            ...
            <table className="familyTable" style={{ width: '100%', textAlign: 'center', marginBottom: '40px' }}>
                <thead>
                    <tr style={{ background: '#eee' }}>
                        <th style={{ width: '20px' }}></th><th>H2. Name</th><th>H3. Relationship to Head</th><th>H4. Civil Status</th><th>H5. Age</th><th>H6. Sex</th><th>H7. Occupation</th><th>H8. Occupation Class</th><th>H9. Disability / Sp. Needs</th><th>H10. Critical Illness</th><th>H11. Solo Parent</th>
                    </tr>
                </thead>
                <tbody>
            ...
            <div className="form-section">
                ...
                {["Makeshift/salvaged/improvised materials", "Mixed but predominantly makeshift/salvaged", "Light materials such as bamboo/sawali/cogon/nipa but not sturdy and durable", "Mixed but predominantly light materials", "Strong materials such as concrete/brick/stone or wood or half galvanized iron and half concrete or galvanized iron/aluminum or glass", "Bamboo/sawali/cogon/nipa but sturdy and durable"].map((opt, i) => (
                    <label className="radio-option" key={i}><input type="radio" name="housing_material" value={opt} checked={val("housing_material") === opt} onChange={handleChange} /> {String.fromCharCode(97 + i)}. {opt}</label>
                ))}
            ...
            <div className="submit-container" style={{ display: 'flex', justifyContent: 'flex-end', gap: '15px', marginTop: '40px', paddingRight: '10%' }}>
                <button type="button" onClick={() => router.push(`/print?id=${docId}`)} style={{ background: '#512da8', color: 'white', padding: '12px 30px', borderRadius: '25px', border: 'none', cursor: 'pointer', fontWeight: 'bold' }}>
                    🖨️ GO TO PRINT FORM
                </button>
